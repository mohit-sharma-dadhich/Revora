...
function VerdictBanner({ decision, measurement }: { decision: string; measurement: ExperimentMeasurement }) { const scale = decision === 'SCALE'; const insufficientData = decision === 'INSUFFICIENT_DATA'; const tone = scale ? 'emerald' : insufficientData ? 'sky' : 'amber'; const toneClasses = { emerald: { border: 'border-emerald/25 bg-emerald/[0.08]', text: 'text-emerald', dot: 'bg-emerald', badge: 'border-emerald/25 bg-emerald/10 text-emerald' }, sky: { border: 'border-sky-500/25 bg-sky-500/[0.07]', text: 'text-sky-300', dot: 'bg-sky-300', badge: 'border-sky-500/25 bg-sky-500/10 text-sky-300' }, amber: { border: 'border-amber-500/25 bg-amber-500/[0.07]', text: 'text-amber-300', dot: 'bg-amber-300', badge: 'border-amber-500/25 bg-amber-500/10 text-amber-300' } }[tone]; const revenueIsPositive = measurement.incremental.incrementalRevenuePerEligibleCustomer > 0; return <motion.section initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.45 }} className={`overflow-hidden rounded-xl border ${toneClasses.border} p-6 sm:p-8`}><div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className={`mb-4 flex items-center gap-2 text-xs font-medium uppercase tracking-[0.16em] ${toneClasses.text}`}><span className={`size-2 rounded-full ${toneClasses.dot}`} />Experiment verdict</div><h1 className={`break-words text-4xl font-bold tracking-[-0.06em] ${toneClasses.text} sm:text-7xl`}>{insufficientData ? 'INSUFFICIENT DATA' : decision}</h1><p className="mt-3 max-w-2xl break-words text-sm leading-6 text-slate-200">{insufficientData ? `More observed conversions are needed before a verdict. Current revenue ${revenueIsPositive ? 'supports scaling in the future' : 'does not support scaling yet'} once enough data is collected.` : scale ? `Treatment generated ${formatMoney(measurement.incremental.incrementalRevenuePerEligibleCustomer)} more per eligible customer than control.` : 'The measured treatment did not clear every decision threshold, so keep the experiment stopped.'}</p></div><Badge className={`w-fit shrink-0 ${toneClasses.badge}`}>{decision}</Badge></div></motion.section> }
