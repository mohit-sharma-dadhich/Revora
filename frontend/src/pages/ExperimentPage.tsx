...
function PaymentPanel({ controlCustomers, treatmentCustomers, paidCustomers, activeCustomer, onPay }: { controlCustomers: string[]; treatmentCustomers: string[]; paidCustomers: Record<string, boolean>; activeCustomer: string | null; onPay: (customerId: string, group: CustomerGroup) => void }) { return <Card><CardHeader><div className="flex items-center gap-3"><CircleDollarSign size={17} className="text-emerald" /><div><p className="text-sm font-medium text-white">Test Mode payments</p><p className="mt-1 text-xs text-muted">Select one or two customers per group to measure the experiment.</p></div></div></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2">{(['control', 'treatment'] as const).flatMap((group) => (group === 'control' ? controlCustomers : treatmentCustomers).map((customerId, index) => { const key = `${group}:${customerId}`; const paid = paidCustomers[key]; const busy = activeCustomer === key; return <div key={key} className="flex items-center justify-between gap-3 rounded-lg border border-line bg-white/[0.02] p-3"><div className="min-w-0"><p className="text-xs font-medium text-slate-200">{group === 'control' ? 'Control' : 'Treatment'} Customer {index + 1}</p><p className="mt-1 truncate font-mono text-[10px] text-muted">{customerId}</p></div><div className="flex shrink-0 items-center gap-2"><span className={paid ? 'rounded-full bg-emerald/10 px-2 py-1 text-[10px] text-emerald' : 'rounded-full border border-line px-2 py-1 text-[10px] text-muted'}>{paid ? 'Paid' : group}</span><Button variant={paid ? 'ghost' : 'outline'} className="h-8 px-2.5 text-xs" disabled={paid || busy} onClick={() => onPay(customerId, group)}>{busy ? 'Opening...' : paid ? 'Verified' : 'Simulate Payment'}</Button></div></div> }))}</CardContent></Card> }
